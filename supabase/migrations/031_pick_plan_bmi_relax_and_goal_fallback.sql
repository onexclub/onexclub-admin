-- Fix plan matching gaps found against MVP templates:
-- 1) BMI band was never relaxed → underweight/overweight members got wrong goal via last resort
-- 2) Last resort ignored intake goal → random same-tier template
-- 3) Eggetarian had no templates → fell through to unrelated diet plan
...
AS $$
    SELECT CASE TRIM(COALESCE(p_label, ''))
        ...
        WHEN 'Eggetarian'           THEN 'vegetarian'
        WHEN 'Pescatarian'          THEN 'non_vegetarian'
        WHEN 'Keto'                 THEN 'keto'
...
DECLARE
    ...
    v_strict    INTEGER;
    v_fallback_goal TEXT;
BEGIN
    v_goals := public.intake_fitness_goal_fallbacks(p_fitness_goal_label);
    v_diet_tag := public.map_diet_type_tag(p_diet_type_label);
    v_fallback_goal := v_goals[1];

    -- strict 0: full match
    -- strict 1: relax age
    -- strict 2: relax gender
    -- strict 3: relax score
    -- strict 4: relax BMI (templates often use normal band only)
    FOREACH v_goal IN ARRAY v_goals LOOP
        FOR v_strict IN 0..4 LOOP
            SELECT pt.* INTO v_plan
             ...
             WHERE pt.plan_type = p_plan_type
               ...
               AND (
                    v_strict >= 4
                    OR (
                        (pt.min_bmi IS NULL OR pt.min_bmi <= COALESCE(p_bmi, 22))
    ...
    -- Last resort: same tier + primary goal from intake (not any random template)
    SELECT pt.* INTO v_plan
      FROM plan_templates pt
     WHERE pt.plan_type = p_plan_type
       AND pt.is_active = TRUE
       AND pt.deleted_at IS NULL
       AND pt.difficulty_level = p_tier
       AND (v_fallback_goal IS NULL OR pt.primary_goal = v_fallback_goal)
       AND (pt.outlet_id = p_outlet_id OR pt.outlet_id IS NULL)
       AND (p_exclude_template_id IS NULL OR pt.id <> p_exclude_template_id)
