-- Goal-aware plan matching: intake `fitness_goal` + `diet_type` must drive template selection.
-- Previous logic ignored primary_goal and ranked only by numeric score proximity,
-- so "Athletic Performance" / "Muscle Gain" often received weight_loss templates.

-- ── Map intake answer labels → plan_templates.primary_goal slugs ─────────────
...
AS $$
    SELECT CASE TRIM(COALESCE(p_label, ''))
        WHEN 'Weight Loss'           THEN ARRAY['weight_loss']::TEXT[]
        WHEN 'Muscle Gain'           THEN ARRAY['muscle_gain', 'general_fitness']::TEXT[]
        WHEN 'Athletic Performance'  THEN ARRAY['muscle_gain', 'general_fitness', 'endurance']::TEXT[]
        WHEN 'Improve Endurance'     THEN ARRAY['endurance', 'general_fitness']::TEXT[]
        WHEN 'Increase Flexibility'  THEN ARRAY['flexibility', 'general_fitness']::TEXT[]
        WHEN 'General Fitness'       THEN ARRAY['general_fitness', 'muscle_gain']::TEXT[]
...
-- ── Pick best template: goal first, then diet tag, then demographics, then score ─
...
BEGIN
        ...
        FOR v_strict IN 0..2 LOOP
            SELECT pt.* INTO v_plan
             ...
             WHERE pt.plan_type = p_plan_type
               ...
               -- Diet preference (diet plans only)
               AND (
                    p_plan_type <> 'diet'
                    OR v_diet_tag IS NULL
                    OR v_diet_tag = ANY (pt.tags)
                    OR pt.name ILIKE ('%' || REPLACE(v_diet_tag, '_', ' ') || '%')
                    OR pt.name ILIKE ('%' || v_diet_tag || '%')
               )
               -- Demographics: tighten → relax across strictness passes
               AND (
                    v_strict >= 1
               ...
               AND (pt.min_score IS NULL OR pt.min_score <= COALESCE(p_score, 0))
               AND (pt.max_score IS NULL OR pt.max_score >= COALESCE(p_score, 0))
             ORDER BY
               CASE WHEN pt.outlet_id IS NOT NULL THEN 0 ELSE 1 END,
               -- Prefer templates whose age/gender explicitly match when available
               CASE
    ...
    -- Last resort: same tier at outlet, any goal (legacy behaviour)
    SELECT pt.* INTO v_plan
      FROM plan_templates pt
     WHERE pt.plan_type = p_plan_type
       AND pt.is_active = TRUE
...
-- ── Wire goal-aware picker into assign_or_rotate_plans ───────────────────────
