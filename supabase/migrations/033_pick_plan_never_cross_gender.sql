-- Never assign gender-specific templates to the opposite sex.
-- Previously v_strict >= 2 relaxed gender, so male members got female-only
-- flexibility plans when no male template existed in the tier/goal band.
...
BEGIN
    ...
    -- strict 0: full match (age + gender + score + BMI)
    -- strict 1: relax age
    -- strict 2: relax score (gender always enforced)
    -- strict 3: relax BMI (gender always enforced)
        ...
        FOR v_strict IN 0..3 LOOP
            SELECT pt.* INTO v_plan
             ...
             WHERE pt.plan_type = p_plan_type
               ...
               -- Gender: never cross-assign (NULL target_gender = unisex only)
               AND (
    ...
    -- Last resort: same tier + primary goal, still gender-safe
    SELECT pt.* INTO v_plan
      FROM plan_templates pt
     WHERE pt.plan_type = p_plan_type
       AND pt.is_active = TRUE
