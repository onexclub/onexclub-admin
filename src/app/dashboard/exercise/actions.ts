...
export async function createExercisePlanAction(formData: FormData): Promise<void> {
  const ctx = await getAuthDashboardContext();
  if (!ctx.user || !hasAccess(ctx.appRole, "exercise_plans", "write")) return;
  ...
  const { error } = await supabase.from("exercise_plans").insert(payload);
