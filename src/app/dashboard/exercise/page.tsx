...
import { createExercisePlanAction } from "@/app/dashboard/exercise/actions";
...
export default async function DashboardExercisePlansPage() {
  ...
  const { data: rows, error } = outletIds.length
    ? await supabase
        .from("exercise_plans")
        .select(
          "id,title,is_template,valid_from,valid_until,outlet_id,is_active,profiles!exercise_plans_profile_id_fkey(email,full_name)",
        )
        .is("deleted_at", null)
  ...
  return (
    <RoleGuard role={ctx.appRole} feature={FEATURE}>
      <div className="space-y-8">
        <div>
          <h2 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50">Exercise plans</h2>
          <p className="mt-2 max-w-3xl text-sm text-zinc-600 dark:text-zinc-400">
            Same schema as diet plans but targets programming JSON; upload PDFs/images to Supabase Storage and paste HTTPS URLs for `attachments_json`.
          </p>
        </div>

        {!outletIds.length ? (
          <EmptyState title="Awaiting branch scope" description="You need an outlet assignment to load exercise plans." />
        ) : error ? (
          ...
          <>
            ...
            <RoleGuard role={ctx.appRole} feature={FEATURE} requireWrite>
              <section className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950/40">
                ...
                <form action={createExercisePlanAction} className="mt-4 grid gap-3 md:grid-cols-2">
